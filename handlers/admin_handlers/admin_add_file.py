...
async def select_category(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        ...
        await bot.send_message(
            ...
            "Выберите категорию, в которую хотите добавить новый пункт: ",
...
async def load_category(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await FSMAdmin.next()
    await callback_query.message.edit_text("Загрузите ваш архив")
...
async def load_file(message: types.Message, state=FSMContext):
    ...
    await bot.send_message(
        message.from_user.id,
        "Окей, файл был успешно загружен, теперь мы переходим к добавлению карточки товара",
    )
    await bot.send_message(
        message.from_user.id, "Теперь дайте название карточки товара:"
    )
...
async def card_name(message: types.Message, state=FSMContext):
    ...
    await FSMAdmin.next()
    await bot.send_message(
        message.from_user.id, "Теперь отправьте мне описание вашей карточки"
    )
...
def register_handlers_admin(dp: Dispatcher):
    dp.register_message_handler(select_category, text="Добавить файл", state=None)
    dp.register_callback_query_handler(load_category, state=FSMAdmin.category)
    dp.register_message_handler(
        load_file, content_types=["document"], state=FSMAdmin.import_file
    )
    ...
    dp.register_message_handler(
        load_photo, content_types=["photo"], state=FSMAdmin.Import_photo
    )
    dp.register_message_handler(cancel, state="*", commands="345")
    dp.register_message_handler(
