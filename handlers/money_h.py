...
async def replenishment(call: types.CallbackQuery):
    await bot.send_message(call.from_user.id, "Пополнение счета!")
...
def register_new_referal(dp: Dispatcher):
    dp.register_callback_query_handler(replenishment, text='add_money')
