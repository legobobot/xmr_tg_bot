...
async def select_file(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(callback_query.from_user.id, "Что вы хотите удалить?", reply_markup=Girl)


async def del_file(callback_query: types.CallbackQuery, state: FSMContext):
    adata.sqlite_del_file(callback_query.data)
    await state.finish()
    await bot.send_message(callback_query.from_user.id, "Лот был успешно удален!")
...
def register_handlers_admin(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(select_file, state=None)
